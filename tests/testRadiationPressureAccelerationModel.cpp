...
TEST_CASE("Compute radiation pressure for complete absorption at 1 AU", "[radiation_pressure]")
{
    ...
    REQUIRE(computeAbsorptionRadiationPressure(energyFlux)
                == Catch::Approx(expectedRadiationPressure).epsilon(tolerance));
}
           ...
           "[radiation_pressure, acceleration, models]")
{
    ...
    REQUIRE(acceleration[0] == Catch::Approx(expectedAcceleration[0]).epsilon(tolerance));
    REQUIRE(acceleration[1] == Catch::Approx(expectedAcceleration[1]).epsilon(tolerance));
    REQUIRE(acceleration[2] == Catch::Approx(expectedAcceleration[2]).epsilon(tolerance));
}
